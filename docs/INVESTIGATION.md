...
where `abandonment_rate_v?_with_X(segment) = count(sessions in segment, version=v?, outcome=abandoned, failure_labels.mode=X) / n_v?(segment)`. This subtracts the rate of X-flagged abandonment that would already be expected under v1 at v2's volume, leaving only the *extra* X-flagged abandoned sessions associated with the version change.
...
**Why this is exact, and what "exact" does and doesn't mean.** Because every abandoned session has exactly one failure label (including `none`), summing `E_X(segment)` over every mode `X` (the full taxonomy, `none` included) reproduces `E(segment)` exactly — the decomposition is a true partition of the observed rate difference, not an approximation. This is what makes it possible to say a mode's share is *of* the regression rather than merely correlated with abandoned sessions in general. It does **not** mean any individual share is bounded to [0%, 100%]: a mode whose rate *decreased* in v2 contributes a negative share (partially offsetting the regression), and when that happens, another mode's share can legitimately exceed 100% to compensate — this is a real, interpretable pattern (one mechanism worsened, another improved, and the worsening one explains more than the net total), not a bug, and the UI shows both the percentage and the raw `E_X` count so this can't be misread as "impossible."

**Terminology.** This is reported as **"share of excess abandonment associated with failure mode X,"** never as "share of abandonment attributable to X" or "caused by X." `failure_labels` are themselves classifier outputs correlated with abandonment, not the result of a controlled intervention on that specific mechanism — establishing that fixing X would actually reduce abandonment by this amount would require an experiment that manipulates X directly (e.g., an A/B test on the clarification policy itself), which is out of scope here. The metric's honest claim is: *of the additional abandoned sessions v2 produced in this segment, this fraction were also disproportionately labeled with failure mode X, net of the baseline rate* — a decomposition of an observed difference, not a causal estimate of X's effect.

**What is still shown for context, unchanged from the earlier draft's intent**: the raw **share of failures** (`count(failure_labels.mode=X, version=v2) / count(failure_labels.mode != 'none', version=v2)`) is displayed next to the share of excess abandonment so the common-but-not-decisive vs. rare-but-decisive contrast remains visible — this was the right instinct in the original design; only the second number's formula and its causal wording needed correcting.
