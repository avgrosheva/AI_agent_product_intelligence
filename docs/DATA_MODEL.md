...
- **Separation of observed behavior from offline evaluation.** `sessions` / `product_events` record what actually happened (behavioral, used for conversion/abandonment). `evaluations` records automated judgments of answer quality ("task success," independent of whether the user bought anything). Collapsing these two would make it structurally impossible to represent task success and conversion moving independently of each other — whether that means the brief's dramatic "task success up, conversion down" illustration, or the milder, genuinely mixed pattern this project's own generated dataset actually produces (`PRD.md` SS2): task success and conversion both roughly flat at the aggregate while several guardrails and specific segments regress.
